...
$sql = "select * from users where category = '2' order by fname, sname desc";
...
if (mysqli_num_rows($result) > 0) {
	...
	echo '</select>&nbsp;&nbsp;<input style="font-family:Century Gothic;font-weight:bold; font-size:15px"type="submit" value ="Downgrade User" ><br><br><br><br></fieldset></form>';
} else {
    echo "<h1 style='background-color:#CCC'>No Admin Users!</h1>";
}

if (isset ($_POST['selection']) && $_POST['selection'] != ""){
	$selection = $_POST['selection'];
	$sql = "UPDATE users SET category = '1' WHERE id = '$selection'";
	$result = mysqli_query($mainconn, $sql);
	header('Location: downgrade2.php');
}
